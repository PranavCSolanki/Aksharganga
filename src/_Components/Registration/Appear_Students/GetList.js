...
export default function GetList() {
  ...
  const GenerateRollNumber = async (exam, district, taluka, selectedCenter) => {
    console.log(exam, district, taluka, selectedCenter);
    if (!exam || !district || !taluka || !selectedCenter) {
      toast.error("Please Select All Fields");
    } else {
      try {
  ...
  const DownloadExcel = (exam, center) => {
    ...
    fetch(`http://localhost:3000/api/print/excel/appear?exam=${exam}&center=${selectedCenter}`, {
        method: 'GET',
    })
    .then(response => {
        if (!response.ok) {
  ...
  const DownloadPDF = (exam, center) => {
    fetch(`${process.env.NEXT_PUBLIC_HOST}/api/print/pdf/appear?exam=${exam}&center=${center}`, {
        method: 'GET',
        headers: {
