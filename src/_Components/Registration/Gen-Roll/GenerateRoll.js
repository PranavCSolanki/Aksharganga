...
export default function GenerateRoll() {
  ...
  const DeleteGeneratedroll = async (exam, taluka, selectedCenter) => {
    if (
      window.confirm(
        ...
        let { data } = await axios.get(
          `${process.env.NEXT_PUBLIC_HOST}/api/registration/generate`,
          {
            params: {
              exam,
              taluka,
              center: selectedCenter, // Ensure the parameter names match
            },
          }
        );
  ...
  const handlePdfDownload = (exam, center) => {
    fetch(`${process.env.NEXT_PUBLIC_HOST}/api/print/pdf/Generatedroll?exam=${exam}&center=${center}`, {
        method: 'GET',
        headers: {
    ...
    .then(response => {
        ...
        return response.blob();  // Retrieve the blob containing the PDF data
    })
    .then(blob => {
  ...
  const DownloadExcel = (exam, center) => {
    ...
    fetch(`http://localhost:3000/api/print/excel/generateroll?exam=${exam}&center=${selectedCenter}`, {
        method: 'GET',
    })
    .then(response => {
        if (!response.ok) {
  ...
  return (
    <>
      ...
      <div className="m-auto">
        <div className={styles.heading2}>Roll Number Wise Student List</div>
      </div>
        ...
        <>
                ...
                <tbody className={styles.tbodys}>
                  {currentDistricts.map((stud, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-2">{stud.mobNo}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="btnwrapper">
            <button
              ...
              onClick={() => DeleteGeneratedroll(exam, taluka, selectedCenter)}
            >
              Clear List
            </button>
          </div>
