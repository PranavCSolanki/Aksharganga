...
export default function State() {
  ...
  const filteredData = data
    .filter((dat) =>
      dat.StudentName.toLowerCase().includes(searchTerm.toLowerCase())
    )
    .sort((a, b) => {
      ...
      return 0; // Default case when sortOrder is "Search By"
    });
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_HOST}/api/rank/displayrank/state`,
        {
          exam: exam,
      ...
      if (response.data.Success) {
        const fetchedData = response.data.data; // Ensure this is the array you expect
        if (Array.isArray(fetchedData)) {
          setColumn(response.data.columns);
          setData(response.data.data);
          toast.success("Data fetched successfully!");
        } else {
          setData([]); // Handle cases where the response isn't an array
  ...
  const handlePdfDownload = async (exam, selectedClass) => {
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_HOST}/api/print/pdf/rank/state?exam=${exam}&standard=${selectedClass}`,
        null,
        {
          headers: {
  ...
  return (
    <>
      <ToastContainer />
      <div className={styles.heading2}>State Level Rank</div>
        ...
        <div className="mb-7" style={{ marginBottom: "10px" }}>
          <div className="relative m-10 mb-6 p-8 transition-all duration-500 hover:shadow-3xl hover:scale-102">
            <button
              ...
              onClick={() => handlePdfDownload(exam, selectedClass)}
            >
              ...
              <tbody className={styles.tbodys}>
                {currentData.map((dat, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-2">{dat.district}</td>
