...
export default function GenerateRoll() {
  ...
  const handlePdfDownload = async (exam, center) => {
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_HOST}/api/print/pdf/generatedroll`,
        {
          exam,
          center,
          sortOrder,
  ...
  const DownloadExcel = async (exam, center) => {
    try {
      const response = await axios.get(
        `http://localhost:3000/api/print/excel/generateroll?exam=${exam}&center=${center}&sortOrder=${sortOrder}`,
        {
          responseType: "blob", // Important for handling binary data
        }
      );
