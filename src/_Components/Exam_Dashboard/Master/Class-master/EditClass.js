import PropTypes from "prop-types";
import { useState } from "react";
import {
  Dialog,
  DialogPanel,
  DialogTitle,
} from "@headlessui/react";
import styles from "../../Styles/Edit.module.css";
import { toast } from "react-toastify";
import axios from "axios";

export default function EditClass({ props, fetchData1 }) {
  const [open, setOpen] = useState(false);
  const [clas, setClas] = useState(props.ClassName);
  const [error, setError] = useState(null);
  const [clasid, setClasid] = useState(props.ClassId);
  const [errorid, setErrorid] = useState(null);
  

  const toggleModal = () => setOpen(!open);

  const Submit = async (e) => {
    e.preventDefault();

    if (!clas) {
      setError("Please Enter Class Name");
      return;
    }
    if (!clasid) {
      setErrorid("Please Enter Class ID");
      return;
    }

    try {
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_HOST}/api/master/classes`,
        { id: props._id, ClassName: clas,clsid: clasid }
      );

      if (response.data.success) {
        toast.success("Class updated successfully!");
      } else {
        toast.error("Failed to update Class");
      }
      fetchData1();
      setOpen(false);
    } catch (error) {
      toast.error("An error occurred while updating the Class");
    }
  };

  return (
    <div>
      <button type="button" onClick={toggleModal} className={styles.edit}>
        Edit
      </button>
      {open && (
        <Dialog
          open={open}
          onClose={() => setOpen(false)}
          className={styles.dialog}
        >
          <div className={styles.dialogContainer}>
            <DialogPanel className={styles.dialogPanel}>
              <div className={styles.dialogContent}>
                <div className={styles.dialogHeader}>
                  <div className={styles.dialogTitleContainer}>
                    <DialogTitle as="h3" className={styles.heading2}>
                      Edit Class 
                    </DialogTitle>
                  </div>
                </div>
                <div className={styles.container}>
                  <label htmlFor="name" className={styles.label}>
                    Enter Class Name
                  </label>
                  <div className="relative">
                    <input
                      id="name"
                      name="name"
                      type="text"
                      value={clas}
                      onChange={(e) => {
                        setClas(e.target.value);
                        setError(null);
                      }}
                      required
                      className="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm px-4 py-3 m-10"
                      placeholder="Enter Class Name"
                    />
                    {error && (
                      <label
                        htmlFor="name"
                        className={styles.label}
                        style={{ color: "red" }}
                      >
                        {error}
                      </label>
                    )}
                  </div>
                </div>
                <div className={styles.container}>
                  <label htmlFor="id" className={styles.label}>
                    Enter Class ID
                  </label>
                  <div className="relative">
                    <input
                      id="id"
                      name="id"
                      type="number"
                      value={clasid}
                      onChange={(e) => {
                        setClasid(e.target.value);
                        setErrorid(null);
                      }}
                      required
                      className="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm px-4 py-3 m-10"
                      placeholder="Enter Class ID"
                    />
                    {errorid && (
                      <label
                        htmlFor="id"
                        className={styles.label}
                        style={{ color: "red" }}
                      >
                        {errorid}
                      </label>
                    )}
                  </div>
                </div>
              </div>
              <div className={styles.dialogFooter}>
                <button
                  type="button"
                  onClick={Submit}
                  className={styles.saveButton}
                >
                  Save
                </button>
                <button
                  type="button"
                  onClick={() => setOpen(false)}
                  className={styles.cancelButton}
                >
                  Cancel
                </button>
              </div>
            </DialogPanel>
          </div>
        </Dialog>
      )}
    </div>
  );
}

EditClass.propTypes = {
  props: PropTypes.shape({
    _id: PropTypes.string.isRequired,
  }).isRequired,
  ClassName: PropTypes.string.isRequired,
  ClassId: PropTypes.number.isRequired,
  _id: PropTypes.object._id,
  fetchData1: PropTypes.func.isRequired,
};
