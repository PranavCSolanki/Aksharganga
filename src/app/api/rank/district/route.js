...
export async function POST(req) {
    ...
    if (!exam || !standard || !district) {
        return NextResponse.json({ "Success": false, message: "exam, district and standard are required" });
    }
    ...
    const stateRankedRollNos = new Set(stateRankData.data.map((item) => item.RollNo));

    // Log state-ranked roll numbers for debugging
    // console.log("State Ranked Roll Numbers:", Array.from(stateRankedRollNos));
    ...
    let districtData = await ResultModel.aggregate([
        {
            $match: {
                ...
                district: district,
                Standard: standard,
                RollNo: { $nin: Array.from(stateRankedRollNos) } // Exclude state-ranked students
            }
        },
        {
            $addFields: {
    ...
    if (!districtData || districtData.length === 0) {
        return NextResponse.json({ "Success": false, message: "No district data found" });
    }
    ...
    if (!topRanks.length) {
        return NextResponse.json({ "Success": false, message: "No data found for top ranks" });
    }
