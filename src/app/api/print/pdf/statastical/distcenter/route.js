...
export async function POST(req) {
  try {
    ...
    const district = searchParams.get("district"); // New district parameter
    ...
    if (district) {
      query.district = district; // Filter by district if provided
    }
    ...
    // Group data by center (previously district)
    const groupedByCenter = {};
    allData.forEach((student) => {
      const center = student.center.trim(); // Use center instead of district
      if (!groupedByCenter[center]) {
        groupedByCenter[center] = Array(standards.length).fill(0);
      }
      const standardIndex = standards.indexOf(student.Class.trim().toLowerCase());
      if (standardIndex >= 0) {
        groupedByCenter[center][standardIndex]++;
      }
    });
    ...
    Object.entries(groupedByCenter).forEach(([center, standardCounts], index) => {
              ...
              <tbody>
                ${tableData.map(
                  ...
                  `<tr>
                    ...
                    <td>${row.totalForCenter}</td>
                  </tr>`
                ).join("")}
                <tr class="total-row">
                  <td>${totalRow.srNo}</td>
                  <td>${totalRow.center}</td>
                  ${totalRow.standardCounts.map((count) => `<td>${count}</td>`).join("")}
                  <td>${totalRow.totalForCenter}</td>
                </tr>
              </tbody>
            </table>
          </div>
